...
def test_invoke_function():
    try:
        ...
        result("Lambda invocation succeeded (StatusCode = 200)",
               status == 200)
    except Exception as e:
...
def test_cloudwatch_logs():
    ...
    try:
        ...
        result("CloudWatch log group exists for Lambda", len(groups) > 0)
    except Exception as e:
        result(f"CloudWatch log check ERROR: {e}", False)
...
def run_test_cases(credentials):
    ...
    print("=" * 60)
    print("LAB 6_1 VALIDATION: GifFinder Lambda Function")
    print("=" * 60)
    test_dynamodb_table()
    test_lambda_role()
