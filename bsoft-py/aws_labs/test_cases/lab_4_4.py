...
EMPLOYEE_1_REQUIRED_ATTRIBUTES = ["Name", "Department", "Country", "Email", "Phone", "Experience", "Status"]
EMPLOYEE_2_ID = "EMP002"
EMPLOYEE_2_EXPECTED_DEPARTMENT = "Human Resources"
...
def test_employee_1_has_required_attributes():
    label = f"'{EMPLOYEE_1_ID}' contains all additional attributes"
    try:
        item = _get_item(EMPLOYEE_1_ID)
        if not item:
            return result(label, False)
        has_all = all(attr in item for attr in EMPLOYEE_1_REQUIRED_ATTRIBUTES)
...
def test_employee_2_department():
    label = f"'{EMPLOYEE_2_ID}' Department = '{EMPLOYEE_2_EXPECTED_DEPARTMENT}'"
    try:
        item = _get_item(EMPLOYEE_2_ID)
        if not item:
            return result(label, False)
        department = item.get('Department', {}).get('S')
        return result(label, department == EMPLOYEE_2_EXPECTED_DEPARTMENT)
    except Exception as e:
        return result(f"{label} - ERROR: {e}", False)
...
def run_test_cases(credentials):
    ...
    print("=" * 60)
    print("LAB 4.4 VALIDATION: Create Items and Additional Attributes in Amazon DynamoDB")
    print("=" * 60)
    ...
    if table_exists:
        ...
        if table_active:
            ...
            if employee_1_exists:
                test_employee_1_has_required_attributes()
            else:
                skip(f"'{EMPLOYEE_1_ID}' contains all additional attributes")

            employee_2_exists = test_employee_2_exists()
            if employee_2_exists:
                test_employee_2_department()
            else:
                skip(f"'{EMPLOYEE_2_ID}' Department = '{EMPLOYEE_2_EXPECTED_DEPARTMENT}'")
