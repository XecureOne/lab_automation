...
def test_target_health(tgs):
    for name, tg in tgs.items():
        if not tg:
            result(f"Targets in '{name}' are healthy", False)
            continue
        try:
            ...
            all_ok  = all(h['TargetHealth']['State'] == 'healthy'
                          for h in healths)
            result(f"All targets in '{name}' are healthy", all_ok)
        except Exception as e:
...
def test_access_logs(alb):
    if not alb:
        return result("ALB access logs enabled", False)
    try:
        r     = elbv2.describe_load_balancer_attributes(
                    LoadBalancerArn=alb['LoadBalancerArn'])
        attrs = {a['Key']: a['Value'] for a in r.get('Attributes', [])}
        result("ALB access logs enabled",
               attrs.get('access_logs.s3.enabled') == 'true')
        result("ALB access logs bucket configured",
               bool(attrs.get('access_logs.s3.bucket')))
    except Exception as e:
        result(f"Access logs check ERROR: {e}", False)
...
def run_test_cases(credentials):
    ...
    print("=" * 60)
    print("LAB 15 VALIDATION: ELB Configuration")
    print("=" * 60)
    alb = get_alb()
    test_alb_state(alb)
