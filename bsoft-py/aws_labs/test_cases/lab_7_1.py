...
def test_artifact_bucket():
    try:
        ...
        if buckets:
            result(f"Beanstalk S3 artifact bucket exists: {buckets[0]}", True)
        else:
            result("Beanstalk S3 artifact bucket exists", False)
    except Exception as e:
...
def run_test_cases(credentials):
    ...
    print("=" * 65)
    print("LAB 7_1 VALIDATION: Elastic Beanstalk Deployment")
    print("=" * 65)
